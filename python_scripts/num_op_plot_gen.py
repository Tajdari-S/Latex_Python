...
def num_op_plot_generator(topo):
    ...
    Coeus_400=list([r.number_of_operation for r in results_Coeus if r.number_of_flows==400].__reversed__())
    Coeus_200=list([r.number_of_operation for r in results_Coeus if r.number_of_flows==200].__reversed__())
    Coeus_100=list([r.number_of_operation for r in results_Coeus if r.number_of_flows==100].__reversed__())

    Cupid_100=list([r.number_of_operation for r in results_Cupid if r.number_of_flows==100].__reversed__())
    Cupid_200=list([r.number_of_operation for r in results_Cupid if r.number_of_flows==200].__reversed__())
    Cupid_400=list([r.number_of_operation for r in results_Cupid if r.number_of_flows==400].__reversed__())
    ...
    plt.xticks(x)
    plt.grid()
    plt.yticks(range(0,max(max([Coeus_400,Cupid_400]))+5000,5000))
    ...
    path_to_store = "manuscripts/figures/experiments_results_figures/".__add__(topo).__add__("/").__add__(topo).__add__("_num_operations.eps")
